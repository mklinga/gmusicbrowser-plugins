...
package GMB::Plugin::NOWPLAYINGPLUS;
use strict;
use utf8;
use Encode;
...
{	my $vbox=Gtk2::VBox->new(::FALSE, 2);
	...
	my $check2=::NewPrefCheckButton(OPT.'SENDPANGOESC',_"Escape Pango characters");
	my $replacetable=::MakeReplaceTable('talydnfc');
	$vbox->pack_start($_,::FALSE,::FALSE,2) for $replacetable,$entry,$preview,$entry2,$check,$check2;
	return $vbox;
}
...
{	my $ID=$::SongID;
	...
	return '' unless @cmd;
	for (@cmd) {
		$_= ::PangoEsc( ::ReplaceFields($ID,$_) );
		if (($::Options{OPT.'SENDPANGOESC'}) and (my $utf8=decode_utf8($_))) {$_=$utf8;}
	}
...
{	my $ID=$::SongID;
	...
	{	my $string=::ReplaceFields($ID,"Title=%t\nArtist=%a\nAlbum=%l\nLength=%m\nYear=%y\nTrack=%n\n");
		$string = ::PangoEsc($string) if ($::Options{OPT.'SENDPANGOESC'});
